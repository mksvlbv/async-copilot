...
function AuthCallbackResolver() {
  ...
  useEffect(() => {
    ...
    async function resolveSession() {
      ...
      try {
        if (code) {
          window.location.replace(`/auth/callback/exchange?code=${encodeURIComponent(code)}&next=${encodeURIComponent(next)}`);
          return;
        }

        if (tokenHash && type) {
          window.location.replace(`/auth/callback/exchange?token_hash=${encodeURIComponent(tokenHash)}&type=${encodeURIComponent(type)}&next=${encodeURIComponent(next)}`);
          return;
        }
        ...
        throw new Error("The magic link callback was incomplete. Request a fresh link and try again.");
      } catch (err) {
        if (!cancelled) {
