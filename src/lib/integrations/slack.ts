        ...
        const res = await fetch(webhookUrl, {
          ...
          headers: {
            ...
            ...(payload.idempotencyKey
              ? { "X-Async-Copilot-Idempotency-Key": payload.idempotencyKey }
              : {}),
          },
          body: JSON.stringify(body),
        });

        if (!res.ok && isRetryableSlackStatus(res.status)) {
          const text = await res.text();
          throw new Error(`Slack webhook returned ${res.status}. ${text || "No response body."}`);
        }
...
function isRetryableSlackStatus(status: number) {
