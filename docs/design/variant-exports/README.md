...
- `index.offline.html` — **fully self-contained version** with local font and icon references (verified working offline, 0 console errors)
- `screenshot.png` — full-page reference screenshot from Variant
- `source-url.txt` — original Variant shared URL + download timestamp

## `_vendor/` — offline assets

All external dependencies downloaded into `_vendor/` so the design system can be inspected and tested without internet:

| Path | Content | Size |
|---|---|---|
| `_vendor/google-fonts/` | 13 `.woff2` files (7 Inter + 6 JetBrains Mono variable font subsets) | ~285 KB |
| `_vendor/google-fonts.css` | Original Google Fonts CSS as fetched | ~20 KB |
| `_vendor/google-fonts-local.css` | Rewritten to reference `./google-fonts/*.woff2` | ~18 KB |
| `_vendor/phosphor/` | 6 weight CSS files (regular/thin/light/bold/fill/duotone) + 18 font files (`.woff2`/`.woff`/`.ttf`) | ~7.7 MB |
| `_vendor/phosphor-web.js` | Original Phosphor loader script (reference only) | 356 bytes |

**How to view a design offline**:
1. Start any local HTTP server rooted at `variant-exports/`:
   - `python -m http.server 8765 --directory docs/design/variant-exports`
   - or `npx serve docs/design/variant-exports`
2. Open `http://localhost:8765/01-landing/index.offline.html`
3. All 6 screens verified working with 0 network failures and 0 console errors (tested in Playwright).
...
- **External deps**: exactly 2 per file — Google Fonts CSS link + Phosphor Icons loader script. Both now mirrored locally in `_vendor/`.
